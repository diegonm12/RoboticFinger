...
//******************************************
// IMPORTANT:
// this code is based in the written code of the book Linux Device Drivers by Greg Kroah-Hartman, in the chapter 13 called USB Drivers.
//*****************************************
...
// table with al the devices that can work with this driver
...
// this methods is use to open the device driver, when it is going to be use
...
// As the name of the function says, this method is use to read the information that give the driver
// to be use tin this case to warn the driver program that the instruction is ready.
...
static ssize_t usb_write(struct file *file, const char __user *user_buffer, size_t count, loff_t *ppos)
{
	...
	printk("Writing message in the file created by the driver!\n"); // printing that we are wirting information to the device in the file
	struct usb_skel *dev;
	int retval = 0;
	struct urb *urb = NULL;
	char *buf = NULL;
...
// it is going to be created a file called RoboticFinger and a number, and this file is going to be use 
